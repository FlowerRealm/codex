...
**Note: This MCP server is still experimental. When using it with Realmx CLI, ensure the CLI version matches the MCP server version.**
...
The user can use [Realmx `.rules`](https://developers.openai.com/codex/local-config#rules-preview) files to define how a command should be handled. The action to take is determined by the `decision` parameter of a matching rule as follows:
...
Because `@flowerrealm/realmx-shell-tool-mcp` intercepts `execve(2)` calls directly, it _always_ knows the full path to the program being executed. In turn, this makes it possible to provide stronger guarantees on how [Realmx `.rules`](https://developers.openai.com/codex/local-config#rules-preview) are enforced.
...
To test out the MCP with a one-off invocation of Realmx CLI, it is important to _disable_ the default shell tool in addition to enabling the MCP so Realmx has exactly one shell-like tool available to it:

```bash
realmx --disable shell_tool \
...
This MCP server is designed to be used with [Realmx](https://developers.openai.com/codex/cli), as it declares the following `capability` that Realmx supports when acting as an MCP client:
...
The Realmx harness (used by the CLI and the VS Code extension) sends such requests to MCP servers that declare the `codex/sandbox-state` capability.
...
See [the README in this repo](https://github.com/openai/codex/blob/main/codex-rs/shell-escalation/README.md) for details.
