...
mod tests {
    ...
    #[test]
    fn thread_initial_messages_append_restored_active_plan_after_history() {
        let thread_id = ThreadId::new();
        let thread = codex_app_server_protocol::Thread {
            ...
            turns: vec![Turn {
                ...
                items: vec![codex_app_server_protocol::ThreadItem::Plan {
                    ...
                    text: "<proposed_plan>history</proposed_plan>".to_string(),
                }],
                status: TurnStatus::Completed,
                error: None,
            }],
            active_plan: Some(codex_app_server_protocol::ThreadActivePlan {
                ...
                raw_markdown: "current".to_string(),
        ...
        match &initial_messages[0] {
            EventMsg::ItemCompleted(ItemCompletedEvent {
                item: TurnItem::Plan(plan),
                ..
            }) => {
                assert_eq!(plan.id, "plan-item-1");
            }
            other => panic!("expected historical plan item first, got {other:?}"),
        }
        match &initial_messages[1] {
            EventMsg::PlanUpdate(update) => {
                assert_eq!(update.explanation.as_deref(), Some("Restored active plan"));
                assert_eq!(update.plan.len(), 1);
                assert_eq!(update.plan[0].id.as_deref(), Some("row-1"));
