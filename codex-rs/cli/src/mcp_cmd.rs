...
/// - `add`    — add a server launcher entry to `~/.realmx/config.toml`
...
async fn run_add(config_overrides: &CliConfigOverrides, add_args: AddArgs) -> Result<()> {
    ...
    match oauth_login_support(&transport).await {
        ...
        McpOAuthLoginSupport::Unknown(_) => println!(
            "MCP server may or may not require login. Run `{}` to login.",
            command_example(&format!("mcp login {name}"))
        ),
    }
...
async fn run_list(config_overrides: &CliConfigOverrides, list_args: ListArgs) -> Result<()> {
    ...
    if entries.is_empty() {
        println!(
            "No MCP servers configured yet. Try `{}`.",
            command_example("mcp add my-tool -- my-command")
        );
        return Ok(());
    }
...
async fn run_get(config_overrides: &CliConfigOverrides, get_args: GetArgs) -> Result<()> {
    ...
    println!(
        "  remove: {}",
        command_example(&format!("mcp remove {}", get_args.name))
    );
