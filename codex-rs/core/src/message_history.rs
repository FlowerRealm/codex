...
//!
//! The history is stored at `~/.realmx/history.jsonl` with **one JSON object per
...
/// Filename that stores the message history inside `~/.realmx`.
...
pub(crate) async fn append_entry(
    ...
    // Resolve `~/.realmx/history.jsonl` and ensure the parent directory exists.
